...
#SBATCH -J computeSFS
#SBATCH -o slurm-log/computeSFS.out
#SBATCH -p bigmem
#SBATCH -e slurm-log/computeSFS.err
#SBATCH --nodes=1
...
taxon=$1
append=$2 # whether or not the angsd output has an appended filename
#windowsize=1000
#step=500
...
# (estimate an SFS with realSFS command, after original angsd has already been done)
...
command2="TEMP/"$taxon""$append".saf $n -P $cpu" 
echo $command2
$angsdir/misc/realSFS $command2 > SFS/"$taxon""$append".sfs
