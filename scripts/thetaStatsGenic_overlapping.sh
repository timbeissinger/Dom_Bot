...
#SBATCH -J thetaStatsGenic_overlapping
#SBATCH -o slurm-log/thetaStatsGenic_overlapping_%j.out
#SBATCH -p bigmem
#SBATCH -e slurm-log/thetaStatsGenic_overlapping_%j.err
#SBATCH --nodes=1
...
command0="/home/beissing/Dom_Bot_Git/TEMP/"$taxon"_genic_10_conditioned.saf $n -P $cpu"
echo $command0
$angsdir/misc/realSFS $command0 > SFS/"$taxon"_genic_10_conditioned.sfs

### Calculate thetas for each site
command1="-bam DATA/LISTS/"$taxon"_list.txt -out THETAS/"$taxon"_"$append" -pest SFS/"$taxon"_genic_10_conditioned.sfs -indF DATA/INBREEDING/"$taxon".indF -doSaf 2 -uniqueOnly 0 -anc DATA/TRIP/TRIP.fa.gz -minMapQ $minMapQ -minQ 20 -nInd $nInd -minInd $minInd -baq 1 -ref /group/jrigrp3/bottleneckProject/genomes/Zea_mays.AGPv3.22.dna.genome.fa -GL $glikehood -P $cpu -rf $regionfile -doThetas 1 -doMajorMinor 1 -doMaf 1 -sites /home/beissing/Dom_Bot_Git/TEMP/intersect.TIL.BKN_genic_10.txt"
echo $command1
